...
```
The shiny app in this package demonstrates the package's utility by allowing users to upload csv files of gene expression data. The gene expression data is correlated with each spot in a sample Visium spatial transcriptomic dataset. The chosen dataset is the 10X Genomics Visium Mouse Brain Dataset. 
To run the shinyApp:
...
The *findCorrelatedCells* function makes use of the [] functions from the [] r package to []

The *visualizeCells* function makes use the [] functions from the [] r package to []
...
Hahsler M, Nagar A (2019). rBLAST: R Interface for the Basic Local Alignment Search Tool. R package version 0.99.2. [https://github.com/mhahsler/rBLAST](https://github.com/mhahsler/rBLAST).
...
This package was developed as part of an assessment for 2022 BCB410H: Applied Bioinformatics course at the University of Toronto, Toronto, CANADA. `ComparePseudogenes` welcomes issues, enhancement requests, and other contributions. To submit an issue, use the GitHub issues. 
